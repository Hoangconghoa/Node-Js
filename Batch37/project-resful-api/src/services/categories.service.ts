...
const getAll = async (query: any) => {
  ...
  if (query && query.keyword && query.keyword !== "") {
    ...
    objectFilters = {
      ...objectFilters,
      CategoriesName: new RegExp(query.keyword, "i"),
    };
  }
  ...
  const count = await Category.countDocuments(objectFilters);
...
const createCategory = async (data: Icatgory) => {
  const result = await Category.create(data);
  return result;
};
const updateCategory = async (id: String, data: Icatgory) => {
